...
function LQS_WeaponPickupBase:DefaultPickupSystem(receivedData)
    ...
    -- Apply the weapon
    if (targetSlot) then
        Player.actor.EquipNewWeaponEntry(receivedData.weaponEntry, 0, true)
...
function LQS_WeaponPickupBase:DefaultDropGearSelected(selection)
    ...
    local playerSlots = Player.actor.slots
    ...
    if (self:IsLargeGear(self.queuePickupData.weaponEntry)) then
        ...
        if (self:CanBeDropped(playerSlots[selection])) then
            self:DropWeapon(Player.actor, playerSlots[selection], true)
        end
    end
